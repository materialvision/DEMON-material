...
#
# Upstream ACE-Step v0.1.7 ships DCW as a three-file split under
# acestep/models/common/. DEMON collapses that into a single module at
# acestep/engine/dcw.py — the class API matches verbatim.
from acestep.engine.dcw import DCWCorrector
...
class AceStepConditionGenerationModel(AceStepPreTrainedModel):
    ...
    def flowedit_generate_audio(self, **kwargs):
        """Flow-edit overlay (#1156) — not vendored in DEMON.

        Upstream ACE-Step v0.1.7 ships ``flow_edit_pipeline`` as a
        companion module under ``acestep/models/common/``. DEMON has
        not vendored it; if you need flow-edit, port that module.
        """
        raise NotImplementedError(
            "flowedit_generate_audio is not vendored in DEMON. "
            "Port acestep/models/common/flow_edit_pipeline.py from "
            "upstream ACE-Step v0.1.7 if you need this path."
        )
